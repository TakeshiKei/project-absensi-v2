...
const PDFButton = ({ hadirData, tidakHadirData, namaKegiatan }) => {
  ...
  const generatePDF = () => {
        ...
        <Page size="A4">
          <View style={styles.container}>
            ...
            <View style={{ marginBottom: "10pt" }}>
              {hadirData.map((row) => (
                <Text key={row.id_upt}>
                  Nomor: {String(row.id_upt).padStart(2, "0")}, Nama: {row.nama_upt}
                </Text>
              ))}
            </View>
    ...
    const pdfBlob = (
      <>
        <PDFDownloadLink document={<MyDocument />} fileName={`output_${moment().format("YYYY-MM-DD_HHmmss")}.pdf`}>
          {({ blob, url, loading, error }) => (
            <button style={styles.downloadButton} disabled={loading} onClick={handleDownload}>
